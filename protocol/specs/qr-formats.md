...
- `"verified"` - every crypto and chain check agrees; a `validity` field further splits this into `"valid"` (both `validFrom` and `validUntil` disclosed, clock inside the window), `"expired"` (past the `validUntil` leaf's UTC end-of-day cutoff), `"not_yet_valid"` (`validFrom` disclosed and still in the future - never collapsed into `"expired"`), `"hidden"` (the presenting device withheld `validFrom` and/or `validUntil` - since neither is one of the seven non-maskable leaves, a presenter may disclose everything else and still leave the one clinician-facing window this stage exists to check unreadable; reported honestly rather than guessed as `"valid"`), or `"revoked"` (the resolved clone's `isValid(root)` is false, checked first and unconditionally - the only way this differs from a false-because-never-issued case, since `not_anchored/root_unset` already covers that one). Full decision procedure: `specs/leaf-commitment.md` section 16, "Reporting validity once binding succeeds".
