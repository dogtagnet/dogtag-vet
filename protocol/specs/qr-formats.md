...
This section covers the mint, verify, wallet-registration, delegation, tag-export, and tag-import session tokens.
The receipt token has its own, deliberately different grammar; see "Receipt URL" below.

Every mint, verify, wallet-registration, delegation, tag-export, or tag-import session token in this ecosystem is 32 lowercase hexadecimal characters: `^[0-9a-f]{32}$`.
That is 128 bits of entropy, base16-encoded, always lowercase on the wire.
A parser MUST reject a mint, verify, wallet-registration, delegation, tag-export, or tag-import token that is the wrong length, contains uppercase hex digits, or contains any character outside `[0-9a-f]`, rather than attempting to normalize it.
