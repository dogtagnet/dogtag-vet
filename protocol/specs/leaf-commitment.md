...
- `recordArtifactVectors` (section 16, WP4.14S) - the `RecordArtifact` sibling of `redactedArtifactVectors` above, over a tree with NO reserved leaves: a full artifact disclosing all seven non-maskable leaves plus two ordinary clinical leaves (`record_full_artifact`), a masked variant withholding one clinical (maskable) leaf with the root proven unchanged (`record_masked_clinical_leaf`), and a variant masking every leaf EXCEPT the seven-member non-maskable set (`record_masked_except_non_maskable`) - the falsifiable demonstration that the non-maskable set, unlike `redactedArtifactVectors`' empty one, is real; plus nine root-preserving negatives, each isolating exactly one check the same way section 15's own promoted negatives do: one per non-maskable keyPath, isolating the non-maskable-set check for that keyPath specifically - `record_negative_masked_dogtagid` (`credentialSubject.dogTagId`), `record_negative_masked_recordtype` (`recordType`), `record_negative_masked_schemaid` (`credentialSchema.id`), `record_negative_masked_schemaversion` (`credentialSchema.version`), `record_negative_masked_chainid` (`issuer.chainId`), `record_negative_masked_contract` (`issuer.contract`), and `record_negative_masked_operator` (`issuer.operator`), each moving that one keyPath's hash to `obfuscatedLeafHashes` with the root unchanged - plus `record_negative_reserved_present` (a hash placed in `reservedLeafHashes` and folded into the posted root, isolating the reserved-must-be-empty check), and `record_negative_overlap` (a genuinely duplicated clinical leaf, one copy disclosed and the other copy's hash obfuscated, isolating the overlap check - the same construction as `negative_overlap_root_preserving_duplicate_leaf`, not the non-isolating `negative_overlap` shape section 15 itself warns against).
  The six additional non-maskable-keyPath negatives beyond `record_negative_masked_dogtagid` (WP4.17A fix round 1 D1) close the gap that left the other six non-maskable keyPaths unpinned by any spec vector: deleting any one of the seven from `RECORD_NON_MASKABLE_KEY_PATHS` now turns exactly one of these nine negatives from `valid: false` (rejected) into a false accept, in both the TS and Rust conformance suites.
