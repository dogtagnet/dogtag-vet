 ...
 */
export function verifyRedactedArtifact(artifact, opts = {}) {
    try {
        ...
        if (!obfuscatedLeafHashes.every(isHex32))
            return false;
        if (reservedLeafHashes.length + disclosed.length + obfuscatedLeafHashes.length > MAX_TOTAL_LEAVES)
            return false;
        if (!isHex32(root))
            return false;
        ...
        const opaque = new Set([...reservedFields, ...obfuscatedFields]);
...
//# sourceMappingURL=redactedArtifact.js.map