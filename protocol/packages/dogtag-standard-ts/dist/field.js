...
export function beToBigInt(bytes) {
    let acc = 0n;
 ...
 */
export function bytesToField(x) {
 ...
 */
export function fieldFromScalarBytes(x) {
    if (x.length > 31)
        throw new Error(`scalar bytes must be <= 31 (got ${x.length})`);
    return beToBigInt(x);
}
/** A small unsigned integer (typeTag, indices) reduced into [0, P). */
export function fieldFromUint(n) {
    const v = BigInt(n);
    if (v < 0n)
        throw new Error("fieldFromUint: negative");
    return v % FIELD_P;
}
...
export function fromHex32(h) {
    const s = h.startsWith("0x") ? h.slice(2) : h;
    const v = BigInt("0x" + s);
    if (v >= FIELD_P)
        throw new Error("hex exceeds field");
    return v;
}
