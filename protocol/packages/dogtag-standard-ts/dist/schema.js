 ...
 */
export function validateSchema(credential) {
    const errs = [];
    const c = credential;
    const push = (msg) => errs.push(msg);
    const reqPresent = (v, path) => {
        if (v === undefined || v === null)
            push(`${path} is required`);
    };
    // --- VC 2.0 envelope ---
    const ctx = c["@context"];
    if (!isArray(ctx)) {
    ...
    const subject = isObject(c.credentialSubject) ? c.credentialSubject : {};
    ...
    const needsChip = isRabies || c.recordType === "EU_HEALTH_CERT" || c.cdcPath === "standard";
    ...
    if (isRabies) {
        ...
        if (subject.titer !== undefined || c.titer !== undefined) {
            const titer = (c.titer !== undefined ? c.titer : subject.titer);
            if (!isObject(titer)) {
