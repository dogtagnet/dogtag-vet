...
import { DS_NODE, poseidon } from "./field.js";
...
export function buildMerkle(leafHashes) {
    if (leafHashes.length === 0)
        throw new Error("buildMerkle: empty leaf set");
 ...
 */
export function merkleProof(layers, leafHash) {
    ...
    for (let l = 0; l < layers.length - 1; l++) {
        ...
        const sib = idx ^ 1;
        if (sib < layer.length)
            proof.push({ sibling: layer[sib] });
        else
            proof.push({ promote: true }); // lone odd node at this level — pass-through
        idx = idx >> 1;
    }
    return proof;
}
 ...
 */
export function processProof(proof, leafHash) {
    let h = leafHash;
 ...
 */
export function verifyInclusion(keyPath, salt, scalar, proof, root) {
