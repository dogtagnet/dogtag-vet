...
export function canonicalInteger(s) {
...
export function canonicalDecimal(s) {
    ...
    if (out.includes(".")) {
        ...
        if (out.endsWith("."))
            out = out.slice(0, -1); // drop trailing dot
    }
    if (out === "-0" || out === "-0.0")
        out = "0";
...
export function assertNotFloat(v) {
...
export function encodeValue(s) {
    switch (s.tag) {
        ...
        default: {
            const _exhaustive = s;
            throw new Error(`unknown tag ${_exhaustive.tag}`);
        }
    }
}
