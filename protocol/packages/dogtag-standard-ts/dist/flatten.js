...
export function flatten(credential) {
    const out = [];
    walk(credential, "", out);
    return out;
}
    ...
    if (typeof node === "object" && node !== null) {
        ...
        for (const k of keys) {
            ...
            if (RESERVED.test(key))
                throw new Error(`reserved char in object key: ${JSON.stringify(key)}`);
            const childPath = path === "" ? key : `${path}.${key}`;
            walk(node[k], childPath, out);
        }
        return;
    }
...
export function tokenizeKeyPath(keyPath) {
    const tokens = [];
    let i = 0;
    let cur = "";
    const flushKey = () => {
    ...
    while (i < keyPath.length) {
        ...
        if (c === ".") {
            ...
            if (!/^(0|[1-9][0-9]*)$/.test(num))
                throw new Error(`bad array index: ${num}`);
        ...
        for (let t = 0; t < tokens.length; t++) {
            ...
            if (tok.kind === "key") {
                ...
                if (last) {
                    ...
                    if (obj[tok.key] === undefined)
                        obj[tok.key] = nextTok.kind === "index" ? [] : {};
                    cursor = obj[tok.key];
                }
            }
            else {
                ...
                if (last) {
                    ...
                    if (arr[tok.idx] === undefined)
                        arr[tok.idx] = nextTok.kind === "index" ? [] : {};
                    cursor = arr[tok.idx];
                }
            }
        }
