...
import { DS_LEAF, bytesToField, fieldFromScalarBytes, fieldFromUint, poseidon } from "./field.js";
import { encodeValue, nfc } from "./encode.js";
const utf8 = new TextEncoder();
export function fieldOfKeyPath(keyPath) {
    return bytesToField(utf8.encode(nfc(keyPath)));
}
export function fieldOfValue(s) {
    return bytesToField(encodeValue(s));
}
/** hashLeaf — Poseidon over the canonical (keyPath, salt, typeTag, value) tuple. */
export function hashLeaf(keyPath, salt, s) {
    if (salt.length !== 16)
