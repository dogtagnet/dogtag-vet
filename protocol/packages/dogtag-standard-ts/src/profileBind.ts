...
import {bytesToField, fromHex32, toHex32, type Field} from "./field.js";
 ...
 *    equals `root`.
 */
export function verifyLeafCommitment(input: VerifyLeafCommitmentInput): boolean {
  try {
    ...
    if (reservedLeafHashes.length + leaves.length > MAX_TOTAL_LEAVES) return false;
    if (!isHex32(root)) return false;

    const seenKeyPathFields = new Set<Field>();
    const recomputed: Field[] = [];
    ...
    for (const leaf of leaves) {
      ...
      if (seenKeyPathFields.has(kpField)) return false; // duplicate opened keyPath
 ...
 */
export function dogTagIdField(handleDec: string): Field {
