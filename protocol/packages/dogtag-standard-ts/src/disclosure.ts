...
import {verifyInclusion, type ProofStep} from "./merkle.js";
import {fromHex32, type Field} from "./field.js";
import {scalarFromPacked} from "./wrap.js";
import {OWNER_IDENTITY_PREFIX, OWNER_NAMESPACE_PREFIX, type TypeTag} from "./types.js";
 ...
 * false.
 */
function rejectOwnerControl(keyPath: string): void {
...
function parseProofSteps(steps: string[]): ProofStep[] {
...
export function verifyProfileDisclosure(d: ProfileDisclosure): boolean {
  ...
  for (const entry of d.disclosures) {
    ...
    if (!verifyInclusion(entry.keyPath, salt, scalar, steps, root)) return false;
  }
  return true;
}
