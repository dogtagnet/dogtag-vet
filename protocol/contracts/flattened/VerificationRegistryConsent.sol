...
/// @notice The `EntityRegistry` authority core (v2 — replaces v1's `ProviderRegistry`), asked one
/// question: may this relayer submit a verification for this purpose? `canVerify` reads the VERIFY axis
/// directly and takes the purpose label rather than a pre-hashed key, so this registry never re-derives
/// the key itself — a hand-mirrored derivation here would be a second source of truth that silently
/// refuses every relayer the moment it drifts from the core's own derivation.
interface IEntityAuthority {
...
/// @notice Owner-hidden verification reads `profileRoot` for the tag<->root binding, `ownerOf` for token
/// EXISTENCE only - never for owner IDENTITY (D1: the tag is minted to a neutral custodian, so `ownerOf`
/// carries no owner meaning and must never gate verification by comparison) - and `status` for the M-1
/// lifecycle gate (uint8-typed here to avoid importing the SBT's own `Status` enum type).
...
contract VerificationRegistryConsent is AccessControlDefaultAdminRules {
    ...
    // M-1 (crypto-improvements-v2.md C1): the two TERMINAL `DogTagSBTConsent.Status` ordinals. Named
    // rather than inlined so a reordered enum fails a drift-guard test instead of silently gating the
    // wrong states - see `test_status_ordinals_match_the_sbt_enum` in `ConsentRegistry.t.sol`.
    uint8 internal constant STATUS_DECEASED = 3;
    ...
    IEntityAuthority public immutable entityRegistry;
        ...
        AccessControlDefaultAdminRules(2 days, admin)
    {
        ...
        entityRegistry = IEntityAuthority(core);
        sbt = IDogTagSBT(sbt_);
    ...
    function recordVerificationZK(
        ...
        if (restrictToApprovedRelayers) {
            require(entityRegistry.canVerify(bytes32(pub[P_PURPOSE]), msg.sender), "!verify-wl");
        }
        ...
        // M-1 (crypto-improvements-v2.md C1): terminal lifecycle states never verify. `Active`, `Lost`
        // and `TransferPending` all pass - an owner proving consent for a LOST or TRANSFER-PENDING tag
        // is legitimate (the tag is still theirs; only a FOUND/CONFIRMED transition, or a fresh custodial
        // issuance, changes that), so only the two terminal states (`Deceased`, `Revoked`) are refused.
        uint8 st = sbt.status(pub[P_DOGTAGID]);
        require(st != STATUS_DECEASED && st != STATUS_REVOKED, "status");
