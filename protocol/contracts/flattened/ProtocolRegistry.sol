...
///
///   * `entityRegistry` — the `EntityRegistry` authority core (v2; replaces v1's `ProviderRegistry`). It
///     occupies the verification registry's immutable `entityRegistry` slot, and it is ALSO where mobile
///     resolves `adminApiUrl`: apps bundle only the ROAX RPC and this `ProtocolRegistry` anchor, read
///     `entityRegistry` off the published discovery set, then call `EntityRegistry.adminApiUrl()` — the
///     v1 doctrine of resolving discovery from one bundled anchor, carried forward.
///
/// There is deliberately NO `providerDirectory` or `serviceDomainResolver` member — v2 retires the whole
/// v1 resolver layer (`ProviderDirectory`, `ServiceDomainResolver`). Directory and domain discovery move
/// off chain, into the admin platform's own database, per architecture-v2.md.
///
...
contract ProtocolRegistry is AccessControlDefaultAdminRules {
    ...
    struct DiscoverySet {
        ...
        address entityRegistry; // == verificationRegistry.entityRegistry(); also where adminApiUrl lives
    ...
    function proposeDiscoverySet(DiscoverySet calldata d) external onlyRole(PUBLISHER_ROLE) {
        ...
        require(d.entityRegistry != address(0), "zero entityRegistry");
