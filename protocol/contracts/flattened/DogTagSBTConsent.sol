...
contract DogTagSBTConsent is ERC721, AccessControlEnumerable, AccessControlDefaultAdminRules, IERC5192 {
    ...
    event StatusChanged(uint256 indexed dogTagId, uint8 from, uint8 to, address by, bytes32 reasonCode);
    ...
    function setStatus(uint256 id, Status s, bytes32 reasonCode) external issuerOrAuthority(id) {
        Status f = status[id];
        if (f == Status.Deceased || f == Status.Revoked) revert Terminal();
        status[id] = s;
        emit StatusChanged(id, uint8(f), uint8(s), msg.sender, reasonCode);
    }
