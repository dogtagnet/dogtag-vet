 ...
 * a standalone script against this repo's own installed viem (2.56.0) and then copied byte-for-byte
 * into both dogtag-vet (here) and dogtag-ios (`DogTagTests/Fixtures/`), where a hand-rolled
 * `Eip712.swift` is vectored against the exact same file. viem is the trusted reference
 ...
 * the fixture) or from any shared module. The two copies can independently drift; that is the
 * point. If a future edit to the generator's field list (order, name, or Solidity type) is not
 * mirrored here, this test's own recomputation disagrees with the checked-in `expected` values
 * and fails - it does not just re-confirm whatever the generator currently believes.
 *
 * JSON encoding note: `message.issuedAt` / `blockNumber` / `deadline` are uint64 in the struct and
 * are stored as DECIMAL STRINGS in the fixture (several vectors hit 2^64-1 =
 * 18446744073709551615, past `Number.MAX_SAFE_INTEGER`). Every parse below goes through `BigInt()`,
 * never `Number()` - the latter would silently round the two uint64-max vectors to the wrong value
 * and this suite would end up "passing" while checking nothing on exactly the vectors that matter
 * most.
 */
...
describe("protocol/specs/eip712-client-registration-vectors.json", () => {
  ...
  it.each(typedVectors.map((v) => [v.name, v] as const))(
    ...
    (_name, vector) => {
      const message = {
        ...
        clientHash: vector.message.clientHash,
        registrationId: vector.message.registrationId,
        wallet: vector.message.wallet,
        issuedAt: BigInt(vector.message.issuedAt),
        blockNumber: BigInt(vector.message.blockNumber),
  ...
  it("covers uint64 max (2^64-1) on each of issuedAt, blockNumber, and deadline individually", () => {
    expect(typedVectors.some((v) => v.message.issuedAt === UINT64_MAX)).toBe(true);
    expect(typedVectors.some((v) => v.message.blockNumber === UINT64_MAX)).toBe(true);
  ...
  it("covers at least one vector where clinic (in-struct) and verifyingContract (domain) differ", () => {
    // Real sessions always set these equal (the spec's "the clone address, redundantly in-struct"),
    // but if every vector here also did, an implementation that read `verifyingContract` for the
    // struct's `clinic` field (or vice versa) would still pass every other vector.
  ...
  it("varies clinic and wallet addresses, and clientHash/registrationId, across every vector", () => {
    ...
    const clientHashes = new Set(typedVectors.map((v) => v.message.clientHash.toLowerCase()));
    const registrationIds = new Set(typedVectors.map((v) => v.message.registrationId.toLowerCase()));
    expect(clinics.size).toBe(typedVectors.length);
    expect(wallets.size).toBe(typedVectors.length);
    expect(clientHashes.size).toBe(typedVectors.length);
    expect(registrationIds.size).toBe(typedVectors.length);
  });
});
