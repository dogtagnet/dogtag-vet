...
"Request a top-up" deep-links to the DogTag admin portal's own status page (`ADMIN_PORTAL_URL`, a new env value) with a `topup` request kind and this wallet's address already filled in (`?kind=topup&wallet=<address>`, `src/lib/topUpRequestUrl.ts`), rather than filing the admin's `topup` operator-request itself: that endpoint is gated by a session scoped to the clinic's admin-portal account, which this app has no way to hold or proxy, so the request itself is still filed by the clinic on that admin portal page - this app's link only prefills it.
