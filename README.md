...
**WP4.15V (this wave): five files (plus the non-code-consumed `events.json`) are mirrored from the `dogtag-protocol` BRANCH `feature/wp4.15-multi-owner` at `e654924`, not from `master`.** `specs/qr-formats.md`, `specs/vet-public-api.yaml`, `specs/events.md`, `contracts/exports/abi/VetIssuer.json` (the branch's v2.1.0 shape), and the brand-new `contracts/exports/abi/DelegationRegistry.json` all carry the WP4.15 multi-owner `/d/` ceremony contract - PLANNED, not deployed on any real chain (Kenneth's own `docs/DEPLOY-wp4.15.md` runbook, on that same branch, is still pending). Every OTHER vendored file (`specs/schemas/*`, `packages/dogtag-standard-ts/`, `contracts/flattened/`, every other ABI, `design/design-system.md`) is untouched by this wave and stays exactly at whatever `master` commit WP4.12V last left it at - this tree has genuinely mixed provenance right now, by design, not by accident. Full accounting in `protocol/PROVENANCE.md`'s own "THIS TREE HAS MIXED PROVENANCE" section, including why a full `scripts/sync-to.sh` run must NOT be run against `master` while this branch is still unmerged (it would silently regress these five files back to their pre-WP4.15 shape).
